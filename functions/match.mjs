...
async function convertGPS(mentorAddress, menteeAddress) {
    const mapsKey = defineSecret("MAPS_KEY").value();
    const query1 = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(mentorAddress)}&key=${mapsKey}`);
    const data1 = query1.data;
    const coordinate1 = data1.results[0].geometry.location;

    const query2 = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(menteeAddress)}&key=${mapsKey}`);
